        ...
        try:
            ...
            if self.playing and self.index < len(self.y_real):
                ...
                if self.y_fcast[self.index] < self.levels[1][0]:
                    ...
                    self.table.setItem(row_num, 5, QTableWidgetItem('Max'))
...
def spanish_inquisition():
    ...
    if (False):
        for t in range(w.ui.spinSize_Y.value()):
            w.ui.add_tab(tabs, t)
            var = setVariables(orders, numbers, x)
            pol = setPolynomials(kind, orders, numbers, x, isMulti)
            lambd = fit(pol, y[:, t])
            psy = setPsy(lambd, numbers, orders, pol, isMulti)
            a = fit(psy, y[:, t])
            fs = setF(a, numbers, psy, isMulti)
            c = fit(fs, y[:, t])
            nev = error(y[:, t], pol, lambd, isMulti)
            coef = fit(var, y[:, t])
            visualize(y[:, t], pol, lambd, t, isMulti)
            beautifulVariable = showVariables(orders, numbers, coef)
            beautifulPolynomial = showAppr(w.ui.comboBox.currentText(), orders, numbers, lambd, isMulti)
            beautifulPsy = showPsy(numbers, a, isMulti)
            beautifulF = showF(numbers, c, isMulti)
            w.ui.resultsText.appendPlainText("ПОМИЛКА: " + str(nev))
            w.ui.resultsText.appendPlainText("НАБЛИЖЕННЯ ЗМІННИМИ: " + str(beautifulVariable))
            w.ui.resultsText.appendPlainText("НАБЛИЖЕННЯ ПОЛІНОМАМИ: " + str(beautifulPolynomial))
            w.ui.resultsText.appendPlainText("НАБЛИЖЕННЯ ФУНКЦІЯМИ ОДНІЄЇ ЗМІННОЇ: " + str(beautifulPsy))
            w.ui.resultsText.appendPlainText("НАБЛИЖЕННЯ ФУНКЦІЯМИ ВЕКТОРНОЇ ЗМІННОЇ: " + str(beautifulF))
            w.ui.resultsText.appendPlainText("--------------------------------------------------------")
            image = QPixmap('./plot' + str(t + 1) + '.png')
            w.ui.image_label.setPixmap(image.scaled(550, 450))
